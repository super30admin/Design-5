...
// Using a Priortity Queue to keep track of the closest parking space available
...
class ParkingLot {
	...
	public void unpark(int floor, int spot) {
		if (floor > maxFloors || spot > maxSpotsPerFloor) {
			throw new IllegalStateException("Not a valid parking spot.");
		}
		ParkingSpot ps = new ParkingSpot(floor, spot);
		if (pq.contains(ps)) {
			System.out.println("Already empty parking spot.");
		}
		pq.add(ps);
	}
